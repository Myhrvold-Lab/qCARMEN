...
def inference_task(
    ...
    for id, group in assay_dict.items():
        ...
        dna_tot_vals = [d[-1] for d in all_sample_res]

        print(f"Shared parameters for group {id}: {shared_res[:NUM_SHARED_PARAMS]}")
        print(f"DNA totals for group {id}: {dna_tot_vals}")
        print(f"DNA vals for group {id}: {dna_vals}")

        print("Wrapping up...")
        mse_vals = get_mses(
            data_normalized, shared_res[:NUM_SHARED_PARAMS].tolist(), all_sample_res
        )
    ...
    with open(f"{local_dir}/dna_totals.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerows(np.expand_dims(all_dna_tots, -1))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to raw data file from Biomark.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Stop optimizer if change in error does not exceed this tolerance over stored history (current history length = 50 iterations).",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=5.0,
        help="Stop optimizer if error is below this threshold.",
    )
    parser.add_argument(
        "--num_iter_multi",
        type=int,
        default=2,
        help="Number of times to run first fitting stage.",
    )
    parser.add_argument(
        "--num_iter_single",
        type=int,
        default=1,
        help="Number of times to run second (individual well) fitting stage.",
    )
    parser.add_argument(
        "--num_reps",
        type=int,
        default=2,
        help="Number of representative samples to use for shared parameter fitting.",
    )
